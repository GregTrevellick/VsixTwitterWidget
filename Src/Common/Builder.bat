...
FOR %%G IN (azure,ch9,code,codeproject,dailydotnettips,dev_humor,dotnet,dotnetcurry,dotnetfdn,dotnetkicks,mscloud,newsycombinator,nodejs,octopusdeploy,reddittech,theregister,visualstudio,vsmdev,vsts,xamarinhq) DO (
	...
	REM move (not copy) vsix's to a single folder, to make easier to upload & purge
	rem xcopy *.vsix %2\Temp\ /F /R /Y
